...
class LexerSuite extends FunSuite with Matchers {
  ...
  def testGoodInputWithLocation(input: String, output: Seq[(SwToken, Int, Int)]) = {
    SwLexer(input) match {
  ...
  def testGoodInput(input: String, output: Seq[SwToken]) = {
    SwLexer(input) should equal (Right(output))
  }

  def testBadInput(input: String, error: LexError) = SwLexer(input) match {
  ...
  implicit class LexerTester(input: String) {
    def ~>(output: Seq[(SwToken, Int, Int)]) = testGoodInputWithLocation(input, output)
    def ~>(output: Seq[SwToken]) = testGoodInput(input, output)
    def ~>(output: SwToken) = testGoodInput(input, Seq(output))
