...
class ParserSuite extends FunSuite with Matchers {
  ...
  def testGoodInput(input: Seq[SwToken], output: Statement) = {
    val parser = new SwParser()
  ...
  def testGoodInput(input: Seq[SwToken], output: Expr) = {
    val parser = new SwParser()
    val reader = parser.reader(input)
  ...
  def testGoodInput(input: Seq[SwToken], output: Ident) = {
    val parser = new SwParser()
    val reader = parser.reader(input)
  ...
  def testBadInput[T](input: Seq[SwToken], parser: statement.type) = {
    val parser = new SwParser()
    val reader = parser.reader(input)
    val result = parser.phrase(parser.statement)(reader)
  ...
  def testBadInput[T](input: Seq[SwToken], parser: expr.type) = {
    val parser = new SwParser()
    val reader = parser.reader(input)
    val result = parser.phrase(parser.expr)(reader)
    result should not be 'successful
  ...
  def testBadInput[T](input: Seq[SwToken], parser: ident.type) = {
    val parser = new SwParser()
    val reader = parser.reader(input)
    val result = parser.phrase(parser.id)(reader)
    result should not be 'successful
  ...
  def testBadInput[T](input: Seq[SwToken], parser: program.type) = {
   ...
   */
  def testGoodProgram(input: Seq[SwToken], output: Seq[Statement]) = {
    SwParser(input) should equal (Right(output))
  }
   ...
   * column.
   */
  def testBadProgram(input: Seq[SwToken], error: ParseError) = SwParser(input) match {
    case Right(output)                          => fail(output.toString ++ " was successful.")
    case Left(SwCompileError(reportedLoc, _)) => {
      (reportedLoc.line, reportedLoc.column) should equal ((error.line, error.column))
    }
  }

  implicit class ParseTester(input: Seq[SwToken]) {
  ...
  test("import.resource") {
    Seq(IMPORT(), FILE("test/library.swav")) ~> Seq(
  ...
  test("import.twice") {
    Seq(IMPORT(), FILE("test/library.swav"), IMPORT(), FILE("test/library.swav")) ~> Seq(
      Assignment(Ident("lib_source"), Seq(Ident("freq"), Ident("amp")), Chain(Seq(
        Application(Ident("fm"), Seq(Var(Ident("freq")), Var(Ident("amp")), Num(4)))
      )))
    )
  ...
  test("import.notFound") {
    Seq(IMPORT(), FILE("test/nosuchfile.swav")) ~/> program
  }
  ...
  def programTest[T](program: String, verify: Seq[SwToken] => T) = SwLexer(program) match {
