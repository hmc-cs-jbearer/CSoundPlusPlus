...
def analyze(output):
    '''
    Parse the output of a run of a SoundWave program to extract useful information.
    ...
    for line in output:
        line = line.strip()
        if line.startswith('SWLOG '):
            fields = [field.strip() for field in line.split(' ')[1:]] # Chop off the "SWLOG" token
