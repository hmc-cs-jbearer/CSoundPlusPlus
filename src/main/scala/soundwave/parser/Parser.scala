...
class SwTokenReader(tokens: Seq[SwToken], lastPos: Location = NoLocation) extends Reader[SwToken] {
  override def first: SwToken = tokens.head
  override def atEnd: Boolean = tokens.isEmpty
  override def pos: Location = tokens.headOption.map(_.loc).getOrElse(lastPos)
  override def rest: Reader[SwToken] =
    ...
    new SwTokenReader(tokens.tail, Location(pos.line, pos.column + 1, pos.file))
}

class SwParser(val disablingContext: SwParser.DisablingContext = SwParser.disablingContext())
  extends Parsers with PackratParsers
{

  override type Elem = SwToken

  def disable(path: String) = require(disablingContext.add(path))

  def isDisabled(path: String) = disablingContext contains path

  def reader(tokens: Seq[SwToken]) = new PackratReader(new SwTokenReader(tokens))

  def apply(tokens: Seq[SwToken]): Either[SwParserError, Seq[Statement]] = {
    phrase(program)(reader(tokens)) match {
      case NoSuccess(msg, next: Input) =>
        Left(new SwParserError(next.pos.asInstanceOf[Location], msg))
      case Success(result, _) => Right(result)
    }
  }

  def located[T <: SwPositional](p: =>Parser[T]): Parser[T] = Parser { in =>
    p(in) match {
  ...
  def sourceFile(file: String): Parser[Seq[Statement]] =
    if (isDisabled(file)) {
      ...
      val ast: Either[SwCompileError, Seq[Statement]] = for {
        source <- SwFileReader(file).right
         ...
         */
        ast <- SwParser(tokens, disablingContext).right
      } yield ast

      ast match {
        case Right(result) => success(result)
        case Left(SwCompileError(pos, msg)) => err(s"Error in file $file ($pos): $msg")
      }
    }
...
object SwParser {
  ...
  def apply(tokens: Seq[SwToken], disabled: DisablingContext = disablingContext()) = {
    val parser = new SwParser(disabled)
    parser(tokens)
  }
